...
class AlbumRepository:
    ...
    def find_by_owner(self, owner_id: int) -> List[Album]:
        """Find all albums owned by a user"""
        try:
            return self.db.query(Album).filter(Album.owner_id == owner_id).all()
        except Exception as e:
            raise ServerError(
                error_code=5000,
                message="Failed to find albums by owner",
                details={"error": str(e)}
            )
