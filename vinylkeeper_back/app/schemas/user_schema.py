...
class UserBase(BaseModel):
    ...
    def validate_timezone(cls, v: str) -> str:
        ...
        if "/" not in v:
            raise ValueError(
                "Invalid timezone format. Must be in IANA format (e.g., Europe/Paris)")
        return v
...
class UserAuthSchema(BaseModel):
    email: EmailStr
    password: str = Field(min_length=4, repr=False)
...
class ResetPasswordSchema(BaseModel):
    token: str
    new_password: str = Field(min_length=4, repr=False)
