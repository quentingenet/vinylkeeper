...
async def get_cover_art(release_id: str) -> dict:
    """
    Fetch cover art JSON from Cover Art Archive.
    Return empty dict on 404.
    """
    client = await get_http_client()
    url = f"{settings.COVER_ART_API}/release/{release_id}"
    try:
        resp = await client.get(url)
        if resp.status_code == 404:
            logger.info(f"No cover art for release {release_id}")
            return {}
        resp.raise_for_status()
        return resp.json()
    except httpx.HTTPError as exc:
        ...
        if status == 404:
            return {}
        logger.error(f"Cover Art API error for {release_id}: {exc}")
        raise ServerError(
            error_code=ErrorCode.RESOURCE_NOT_FOUND,
            message="Failed to fetch cover art",
            details={"error": str(exc)},
        )
...
async def get_discogs_artist(artist_id: str) -> dict:
    ...
    headers = {"Authorization": f"token={settings.DISCOGS_API_KEY}"}
    try:
