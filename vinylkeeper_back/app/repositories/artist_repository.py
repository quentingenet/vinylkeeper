...
class ArtistRepository:
    ...
    def find_by_owner(self, owner_id: int) -> List[Artist]:
        """Find all artists owned by a user"""
        try:
            return self.db.query(Artist).filter(Artist.owner_id == owner_id).all()
        except Exception as e:
            raise ServerError(
                error_code=5000,
                message="Failed to find artists by owner",
                details={"error": str(e)}
            )
