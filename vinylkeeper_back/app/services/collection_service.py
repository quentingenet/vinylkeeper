...
class CollectionService:
    ...
    def add_album_to_collection(self, user_id: int, collection_id: int, album_data: CollectionAlbumCreate) -> CollectionAlbumMetadataResponse:
        ...
        try:
            ...
            collection_album = CollectionAlbum(
                ...
                acquisition_date=album_data.acquisition_date,
                purchase_price=album_data.purchase_price
            )

            created = self.collection_album_repository.create(collection_album)
            return CollectionAlbumMetadataResponse.model_validate(created)
    ...
    def update_album_metadata(self, user_id: int, collection_id: int, album_id: int, metadata: CollectionAlbumUpdate) -> CollectionAlbumMetadataResponse:
        ...
        try:
            ...
            collection_album = self.collection_album_repository.find_by_collection_and_album(
            ...
            if metadata.acquisition_date is not None:
                collection_album.acquisition_date = metadata.acquisition_date
            if metadata.purchase_price is not None:
                collection_album.purchase_price = metadata.purchase_price

            # Save changes
            self.repository.db.commit()
            self.repository.db.refresh(collection_album)

            return CollectionAlbumMetadataResponse.model_validate(collection_album)
        except (ResourceNotFoundError, ForbiddenError):
            raise
        except Exception as e:
            raise ServerError(
        ...
        try:
            ...
            for collection_album in collection.collection_albums:
                album_data = {
                    ...
                    "state_record": collection_album.state_record,
                    "state_cover": collection_album.state_cover,
                    "acquisition_date": collection_album.acquisition_date,
                    "purchase_price": collection_album.purchase_price,
                    "created_at": collection_album.album.created_at,
                    "updated_at": collection_album.album.updated_at,
                    "collections_count": 0,  # You might want to calculate this
                    "loans_count": 0,  # You might want to calculate this
                    "wishlist_count": 0  # You might want to calculate this
                }
