...
class ImageFetcher:
    ...
    def __init__(self, token: str, base_url: str, http_client: Optional[httpx.AsyncClient] = None):
        self.token = token
    ...
    async def _make_request(self, url: str, client: Optional[httpx.AsyncClient] = None) -> Dict[str, Any]:
        ...
        response = await client_to_use.get(url, headers=self.headers)
...
class SearchService:
    ...
    def __init__(self, http_client: Optional[httpx.AsyncClient] = None):
        ...
        self.http_client = http_client
        self.image_fetcher = ImageFetcher(self.token, self.base_url, http_client)
    ...
    async def _search_discogs(self, query: str, entity: str, client: Optional[httpx.AsyncClient] = None) -> List[dict]:
    ...
    async def search_music(self, search_query: SearchQuery) -> List[DiscogsData]:
        ...
        try:
            ...
            response_data = await self._search_discogs(q, entity)
    ...
    async def get_artist_metadata(self, artist_id: str) -> ArtistMetadata:
        ...
        try:
            response = await self.http_client.get(url, headers={"Authorization": f"Discogs token={self.token}"})
            response.raise_for_status()
            data = response.json()
        ...
        artist_metadata = ArtistMetadata(
            ...
            image_url=data.get("images", [{}])[0].get("uri", "") if data.get("images") else "",
    ...
    async def get_album_metadata(self, album_id: str) -> AlbumMetadata:
        ...
        raw_image_url = data.get("images", [{}])[0].get("uri", "") if data.get("images") else ""
